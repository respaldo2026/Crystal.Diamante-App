  ...
  export default function StudentDetailView() {
    ...
    const cargarDatosCompletos = useCallback(async () => {
      ...
      try {
        ...
        const listaMats = dataMatriculas || [];
        setMatriculas(listaMats);

        const activas = listaMats.filter((m: any) => m.estado === "activo").length;
        const finalizadas = listaMats.filter((m: any) => m.estado === "finalizado").length;
        const totalPagado = listaMats.reduce(
          (sum: number, m: any) => sum + (m.monto_pagado || 0),
          0
        );
        const deudaTotal = listaMats.reduce(
          (sum: number, m: any) => sum + (m.deuda_pendiente || 0),
          0
        );
    ...
    const columnasCursos = useMemo(
      () => [
        ...
        },
        {
          ...
          key: "estado",
          render: (estado: string) => {
            const config: Record<string, { color: string; icon: React.ReactNode | null }> = {
              activo: { color: "blue", icon: <CheckCircleOutlined /> },
              finalizado: { color: "green", icon: <CheckCircleOutlined /> },
              cancelado: { color: "red", icon: <CloseCircleOutlined /> },
            };
            const visual = config[estado] || { color: "default", icon: null };
            return (
              <Tag color={visual.color} icon={visual.icon}>
                {estado}
              </Tag>
        ...
        },
        {
          ...
          key: "nota",
          render: (nota: number) =>
            nota ? <Tag color="purple">{nota}</Tag> : <Text type="secondary">Pendiente</Text>,
        },
        {
          title: "Profesor",
          dataIndex: ["cursos", "perfiles", "nombre_completo"],
    ...
    const columnasFinanciero = useMemo(
      () => [
        { title: "Curso", dataIndex: ["cursos", "nombre"], key: "curso" },
        {
          title: "Precio Total",
          dataIndex: ["cursos", "precio"],
          key: "precio",
    ...
    const columnasPagos = useMemo(
      () => [
        ...
        },
        {
          ...
          dataIndex: "metodo_pago",
          key: "metodo",
          render: (val: string) => <Tag>{val || "No especificado"}</Tag>,
        },
        { title: "Referencia", dataIndex: "referencia", key: "ref" },
        { title: "Observaciones", dataIndex: "observaciones", key: "obs" },
      ],
      []
    );
    ...
    return (
      <Show
        title={`Expediente Completo: ${perfil?.nombre_completo}`}
        headerButtons={() => (
          <Button onClick={() => router.push("/estudiantes")}>
            ← Volver a Lista
          </Button>
          ...
          <Col xs={24} sm={8}>
            <Card>
              <Statistic
                ...
                prefix="$"
                valueStyle={{
                  color: estadisticasGlobales.deudaTotal > 0 ? "#ff4d4f" : "#52c41a",
                }}
              />
              {estadisticasGlobales.deudaTotal > 0 && (
                <Tag color="red" style={{ marginTop: 8 }}>
